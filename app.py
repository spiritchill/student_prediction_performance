...
st.caption("Predicts a student's final grade (G3, out of 20) and pass/fail outcome from academic & lifestyle factors.")
...
with col1:
    ...
    address = st.selectbox("Address", ["U", "R"], format_func=lambda x: "Urban" if x == "U" else "Rural")
    famsize = st.selectbox("Family Size", ["LE3", "GT3"], format_func=lambda x: "≤3" if x == "LE3" else ">3")
    Pstatus = st.selectbox("Parents' Cohabitation", ["T", "A"], format_func=lambda x: "Together" if x == "T" else "Apart")
...
if st.button("Predict Performance", type="primary", use_container_width=True):
    ...
    predicted_grade = float(reg_model.predict(scaler.transform(row))[0]) \
        if hasattr(reg_model, "coef_") else float(reg_model.predict(row)[0])
    predicted_grade = max(0, min(20, predicted_grade))

    pass_pred = clf_model.predict(row)[0]
    pass_proba = clf_model.predict_proba(row)[0][1] if hasattr(clf_model, "predict_proba") else None

    c1, c2 = st.columns(2)
    with c1:
        st.metric("Predicted Final Grade (G3)", f"{predicted_grade:.1f} / 20")
    with c2:
        st.metric("Predicted Outcome", "✅ Pass" if pass_pred == 1 else "❌ Fail",
                   delta=f"{pass_proba*100:.1f}% confidence" if pass_proba is not None else None)

    if predicted_grade < 10:
        st.warning("This student profile is at risk of underperforming. Consider recommending extra support, tutoring, or reduced absences.")
    else:
        st.success("This student profile suggests solid academic performance.")

st.divider()
with st.expander("ℹ️ About this project"):
    st.write("""
    This tool uses two machine learning models trained on student demographic,
    family, and lifestyle data (inspired by the UCI Student Performance dataset structure):
    - **Regression model** predicts the exact final grade (G3, 0–20 scale)
    - **Classification model** predicts a binary Pass/Fail outcome

    Both models were evaluated using proper metrics (RMSE/R² for regression,
    Precision/Recall/F1 for classification) rather than relying on a single number.
    """)
